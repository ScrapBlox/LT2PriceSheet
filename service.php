...
$url = "https://sheets.googleapis.com/v4/spreadsheets/$spreadsheetId/values/Service?key=$apiKey";
...
<head>
    ...
    <title>Services - LT2 Pricing Sheet</title>
            ...
            if (isset($data['values'])) {
                ...
                foreach ($data['values'] as $rowIndex => $row) {
                    ...
                    if (count($row) == 1) { // Section headers like "< GENERAL >"
                        if ($ShowAvgPosts == "true") { $rowLength = 3; } else { $rowLength = 2; }
